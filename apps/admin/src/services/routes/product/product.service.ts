...
export class ProductService {
  ...
  async createProduct(product: Product): Promise<ProductServer> {
    ...
    return withTokenRefresh(this.http, (headers) =>
      this.http.post<{ message: string; product: ProductServer }>(
        ApiRoutes.ADMIN.PRODUCT.CREATE_PRODUCT,
        body,
        { headers }
      ).pipe(
        map((response: any) => {
  ...
  async getProductById(productId: string): Promise<ProductServer> {
    return withTokenRefresh(this.http, (headers) =>
      this.http.get<{ product: ProductServer }>(
        ApiRoutes.ADMIN.PRODUCT.GET_PRODUCT_BY_ID(productId),
        { headers }
      ).pipe(
        map((response: any) => {
          if (response.product && response.product.id) {
  ...
  async getAllProducts(creatorId: string): Promise<ProductServer[]> {
    return withTokenRefresh(this.http, (headers) =>
      this.http.get<{ products: ProductServer[] | null }>(
        ApiRoutes.ADMIN.PRODUCT.GET_ALL_PRODUCTS(creatorId),
        { headers }
      ).pipe(
        map((response: any) => {
          if (response && response.products !== undefined && response.products !== null) {
  ...
  async updateProduct(product: Product): Promise<void> {
    ...
    return withTokenRefresh(this.http, (headers) =>
      this.http.patch<ProductServer>(
        ApiRoutes.ADMIN.PRODUCT.UPDATE_PRODUCT(product.id),
        body,
        { headers }
      ).pipe(
        map(() => {
  ...
  async deleteProduct(productId: string): Promise<void> {
    return withTokenRefresh(this.http, (headers) =>
      this.http.delete<any>(
        ApiRoutes.ADMIN.PRODUCT.DELETE_PRODUCT(productId),
        { headers }
      ).pipe(
        map((response: any) => {
          if (response.message) {
  ...
  async addProductImage(productId: string, filePath: string): Promise<{ image: ProductImage; message: string }> {
    ...
    return withTokenRefresh(this.http, () =>
      this.http.post<{ image: ProductImage; message: string }>(
        ApiRoutes.ADMIN.PRODUCT.ADD_PRODUCT_IMAGE,
        formData,
        { headers }
      ).pipe(
        map((response: any) => {
  ...
  async getImageById(productId: string, imageId: string): Promise<Blob> {
    return withTokenRefresh(this.http, (headers) =>
      this.http.get(
        ApiRoutes.ADMIN.PRODUCT.GET_IMAGE_BY_ID(productId, imageId),
  ...
  async getAllImages(productId: string): Promise<{ images: ProductImage[]; product: ProductServer }> {
    return withTokenRefresh(this.http, (headers) =>
      this.http.get<{ images: ProductImage[]; product: ProductServer }>(
        ApiRoutes.ADMIN.PRODUCT.GET_ALL_IMAGES(productId),
        { headers }
      ).pipe(
        map((response: any) => {
          if (response.images && response.product) {
