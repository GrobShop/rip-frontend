...
//         .post(ApiRoutes.LOGIN.LOGIN, data)
//         .pipe(
//           map((res: any) => {
...
//         await StoreService.save(StoreKeys.COMPANY_NAME, response.user.company_name); // Добавлено для роли Partner
//         await StoreService.save(StoreKeys.INN, response.user.inn); // Добавлено для роли Partner
//         await StoreService.save(StoreKeys.CONTACT_PERSON, response.user.contact_person); // Добавлено для роли Partner
//         await StoreService.save(StoreKeys.PHONE, response.user.phone); // Добавлено для роли Partner
//         await StoreService.save(StoreKeys.ADDRESS, response.user.address); // Добавлено для роли Partner
//         ToastService.success('Вход выполнен успешно!');
//
//         // Перенаправление в зависимости от роли
//         setTimeout(async () => {
//           if (response.user.role === 'Partner') {
//             await this.router.navigate(['/partner-dashboard']); // Предполагаемый маршрут для партнёра
//           } else {
//             await this.router.navigate(['/user-dashboard']); // Предполагаемый маршрут для других ролей
//           }
//         }, 200);
//       } else {
