...
export class CartService {
  ...
  async getCartItems(cartId: string): Promise<CartItem[]> {
    return withTokenRefresh(this.http, (headers) =>
      this.http.get<{ items: CartItem[] | null }>(ApiRoutes.STANDARD.CART.GET_ITEMS(cartId), { headers }).pipe(
        map((response: any) => {
          if (response && response.items !== undefined) {
            return Array.isArray(response.items) ? response.items as CartItem[] : [];
          } else if (response.error) {
            throw new Error(response.error);
          } else {
            throw new Error('Некорректный ответ сервера');
  ...
  // 6. SUBMIT CART ORDER
  async submitCartOrder(paymentMethod: string, deliveryMethod: string): Promise<SubmitCartOrderResponse> {
    const body = { payment_method: paymentMethod, delivery_method: deliveryMethod };
